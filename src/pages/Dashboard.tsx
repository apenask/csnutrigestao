...
import { formatCurrency, formatDateTime, isToday, isThisMonth, getLast7Days } from '../utils/dateUtils';

export function Dashboard() {
  const { state } = useStore();

  // Calculate KPIs
  const todaySales = state.sales.filter(sale => isToday(sale.date));
  const monthSales = state.sales.filter(sale => isThisMonth(sale.date));
          ...
          <div className="space-y-3">
            {recentActivities.length === 0 ? (
              ...
              recentActivities.map((activity) => (
                <div 
                  key={activity.id} 
                  className="flex items-center justify-between p-3 card-secondary rounded-lg"
                >
                  <div>
                    <p className="font-medium" style={{ color: 'var(--text-primary)' }}>
                      {'items' in activity ? `Venda #${activity.id}` : activity.description}
                    </p>
                  ...
                  <div className="text-right">
                    <p className={`font-semibold ${
                      'items' in activity || activity.type === 'income' || activity.type === 'sale'
                        ? 'text-green-600' 
                        : 'text-red-600'
                    }`}>
                      {'items' in activity 
